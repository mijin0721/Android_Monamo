...
public class MemoStorage {
    ...
    public String getPlanTotalToDay(int year, int month, int day) {
        int planTotal = 0;
        for (int i = 0; i <= day; i++) {
            String selectedDate = formatDate(year, month, i);
            String memo = getMemo(selectedDate);
    ...
    public int getPlanDate(int year, int month, int date) {
    ...
    public int getCarryDate(int year, int month, int date) {
        String selectedDate = formatDate(year, month, date);
        String memo = getMemo(selectedDate);
        if (memo.isEmpty()) {
            return 0;
